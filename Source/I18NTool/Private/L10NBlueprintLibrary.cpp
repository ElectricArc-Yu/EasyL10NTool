                                              ...
                                              const ELanguage InLanguage, UObject* WorldContextObject)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[L10NTool] Try to get a unsupported language : %s"),
		       ELanguageToString(InLanguage));
		UE_LOG(LogTemp, Warning,
		       TEXT(
			       "[L10NTool Hint] If you want support a language add SupportedLanguage in Project Settings -> Easy I18NTool -> Easy L10N Tool -> Supported Language"
		       ));
	}
	if (InL10NData->GetRowStruct() != FL10NPlainTextTable::StaticStruct())
	{
		UE_LOG(LogTemp, Error, TEXT("[L10NTool] The DataTable %s is not a valid L10N Plain Text DataTable"),
		       *InL10NData->GetName());
	...
	if (!Row)
	{
		UE_LOG(LogTemp, Error, TEXT("[L10NTool] The Row %s is not found in DataTable %s"), *InRowName.ToString(),
		       *InL10NData->GetName());
		UE_LOG(LogTemp, Error, TEXT("[CallStack] \n%s[StackEnd]"), *FFrame::GetScriptCallstack());
...
#pragma endregion
	if (Result.IsEmpty())
	{
		UE_LOG(LogTemp, Error, TEXT("[L10NTool] The Row %s in DataTable %s is not support language %s"),
		       *InRowName.ToString(), *InL10NData->GetName(), ELanguageToString(InLanguage));
		UE_LOG(LogTemp, Error, TEXT("[CallStack] \n%s[StackEnd]"), *FFrame::GetScriptCallstack());
                                                                   ...
                                                                   UObject* WorldContextObject)
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("[L10NTool] The DataTable %s is not a valid L10N Assets DataTable"),
		       *InL10NData->GetName());
		UE_LOG(LogTemp, Error, TEXT("[CallStack] \n%s[StackEnd]"), *FFrame::GetScriptCallstack());
		return nullptr;
	}
...
/**
 * Get L10N Assets Object from DataTable
 * @brief If the language row is empty, it will return the default language row automatically.
 * @param InRowName : Row Name
 * @param InL10NData : Must be a valid L10N Assets DataTable
 * @param InLanguage : Language
 * @param WorldContextObject : The world context
 * @return UObject : L10N Assets Object or nullptr in InL10NData is not a valid L10N Assets DataTable or "Cannot Find Row" in InRowName is not found in InL10NData Row Name.
 */
UObject* UL10NBlueprintLibrary::GetL10NAssetsObject(const FName& InRowName, const UDataTable* InL10NData,
	const ELanguage InLanguage, UObject* WorldContextObject)
{
		CheckSupport(InLanguage, WorldContextObject);
	if (InL10NData->GetRowStruct() != FL10NAssetsTable::StaticStruct())
...
void UL10NBlueprintLibrary::SetTextWithL10N(UObject* WorldContextObject, UWidget* InWidget, const FName& InRowName,
                                            const UDataTable* InL10NData,
                                            const ELanguage InLanguage)
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("[L10NTool] The Widget %s is not a TextBlock or RichTextBlock"),
		       *InWidget->GetName());
		UE_LOG(LogTemp, Error, TEXT("[CallStack] \n%s[StackEnd]"), *FFrame::GetScriptCallstack());
	}
}
...
void UL10NBlueprintLibrary::PlaySound2DWithL10N(const FName& InRowName, const UDataTable* InL10NData,
                                                ...
                                                float StartTime, USoundConcurrency* ConcurrencySettings,
                                                const AActor* OwningActor, bool bIsUISound)
{
	if (USoundCue* SoundCue = GetL10NAudioFromAssetsData(InRowName, InL10NData, InLanguage, WorldContextObject))
	{
		UGameplayStatics::PlaySound2D(WorldContextObject, SoundCue, VolumeMultiplier, PitchMultiplier, StartTime,
		                              ConcurrencySettings, OwningActor, bIsUISound);
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("[L10NTool] The Row %s in DataTable %s is not a valid USoundCue"),
		       *InRowName.ToString(), *InL10NData->GetName());
		UE_LOG(LogTemp, Error, TEXT("[CallStack] \n%s[StackEnd]"), *FFrame::GetScriptCallstack());
	}
}
...
void UL10NBlueprintLibrary::PlaySoundAtLocationWithL10N(const FName& InRowName, const UDataTable* InL10NData,
                                                        const ELanguage InLanguage, UObject* WorldContextObject,
                                                        FVector Location,
                                                        FRotator Rotation, float VolumeMultiplier,
                                                        ...
                                                        const UInitialActiveSoundParams* InitialParams)
{
	if (USoundCue* SoundCue = GetL10NAudioFromAssetsData(InRowName, InL10NData, InLanguage, WorldContextObject))
	{
		UGameplayStatics::PlaySoundAtLocation(WorldContextObject, SoundCue, Location, Rotation, VolumeMultiplier,
		                                      PitchMultiplier, StartTime, AttenuationSettings, ConcurrencySettings,
		                                      OwningActor, InitialParams);
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("[L10NTool] The Row %s in DataTable %s is not a valid USoundCue"),
...
void UL10NBlueprintLibrary::PlayDialogue2DWithL10N(const FName& InRowName, const UDataTable* InL10NData,
                                                   ...
                                                   UObject* WorldContextObject, float VolumeMultiplier,
                                                   float PitchMultiplier,
                                                   float StartTime)
{
	if (UDialogueWave* DialogueWave = GetL10NDialogueFromAssetsData(InRowName, InL10NData, InLanguage,
	                                                                WorldContextObject))
	{
		FDialogueContext* TempContext = new FDialogueContext();
		TempContext->Speaker = Speaker;
		TempContext->Targets = Targets;
		UGameplayStatics::PlayDialogue2D(WorldContextObject, DialogueWave, *TempContext, VolumeMultiplier,
		                                 PitchMultiplier, StartTime);
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("[L10NTool] The Row %s in DataTable %s is not a valid UDialogueWave"),
		       *InRowName.ToString(), *InL10NData->GetName());
		UE_LOG(LogTemp, Error, TEXT("[CallStack] \n%s[StackEnd]"), *FFrame::GetScriptCallstack());
	}
}
                                                           ...
                                                           USoundAttenuation* AttenuationSettings)
{
	                                                                ...
	                                                                WorldContextObject))
	{
		...
		TempContext->Speaker = Speaker;
		TempContext->Targets = Targets;
		UGameplayStatics::PlayDialogueAtLocation(WorldContextObject, DialogueWave, *TempContext, Location, Rotation,
...
void UL10NBlueprintLibrary::SetImageWithL10N(const FName& InRowName, const UDataTable* InL10NData,
                                             const ELanguage InLanguage, UObject* WorldContextObject, UImage* InImage,
                                             const FSlateBrush& InBrushSetting)
{
	if (UTexture2D* Texture2D = GetL10NTextureFromAssetsData(InRowName, InL10NData, InLanguage, WorldContextObject))
	{
		...
		UObject* Obj = Cast<UObject>(Texture2D);
		TempBrush->SetResourceObject(Obj);
		InImage->SetBrush(*TempBrush);
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("[L10NTool] The Row %s in DataTable %s is not a valid UTexture2D"),
		       *InRowName.ToString(), *InL10NData->GetName());
		UE_LOG(LogTemp, Error, TEXT("[CallStack] \n%s[StackEnd]"), *FFrame::GetScriptCallstack());
	}
}
...
{
	if (ALocalizationedGameMode* GameMode = Cast<ALocalizationedGameMode>(
